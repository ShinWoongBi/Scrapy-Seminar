...
class NewsCrawlPages(scrapy.Spider):
    name = "news_pages"
    pages = 1  # 수집할 페이지 수
    url = "https://news.naver.com/main/list.nhn?mode=LS2D&sid2=259&sid1=101&page="  # 데이터 수집할 페이지 URL
    start_urls = [
        "https://news.naver.com/"
    ]

    def parse(self, response):
        # 1 ~ self.pages+1 만큼 반복한다.
        for i in range(1, self.pages + 1):
            yield scrapy.Request(url=self.url + str(i), callback=self.parse_page)

    def parse_page(self, response):
        # <ul class='type06_headline'>에 있는 기사들을 가져온다.
        for li in response.xpath("//ul[@class='type06_headline']/li"):
            url = li.xpath("dl/dt[1]/a/@href").get()
            imaegUrl = li.xpath("dl/dt[@class='photo']/a/img/@src").get()
            author = li.xpath("dl/dd/span[@class='writing']/text()").get()
            meta = {'imaegUrl': imaegUrl, 'url': url, 'author': author}
